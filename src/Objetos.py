...
class Objetos:
    ...
    def detectar(self):
        ...
        sobel_map["edge_sobel_combined"] = cv2.addWeighted(sobel_map["edge_sobel_x"], 0.5, sobel_map["edge_sobel_y"],
                                                           0.5, 0)
        ...
        for coin in coins[0]:
            ...
            cv2.putText(result, coins_map[key]["name"], (x_coor, y_coor), font, fontScale, fontColor, thickness,
                        cv2.LINE_AA)
        ...
        plt.title("Imagen con dados detectados")
        plt.show(block=True)
        ...
        for dice in dices:
            ...
            for val in vals[0]:
                ...
                x_coor, y_coor, detected_radius = int(x_coor), int(y_coor), int(detected_radius)
                dice["img"] = cv2.circle(dice["img"], (x_coor, y_coor), detected_radius, (100, 0, 0),
                                         thickness=cv2.FILLED)

            dice_value = len(vals[0])
            cv2.putText(final_result_dices, f"Value {dice_value}", dice["coord"][0], font, fontScale, fontColor,
                        thickness, cv2.LINE_AA)
            cv2.putText(img_rgb_copy, f"Value {dice_value}", dice["coord"][0], font, fontScale, fontColor, thickness,
                        cv2.LINE_AA)

            sub += 1
            plt.subplot(sub)
            plt.imshow(dice["img"], cmap="gray")
            plt.title(f"Dado {dice_value}")
        plt.show(block=True)
